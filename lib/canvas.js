function clamp(v, min, max) {
  return v < min ? min : v > max ? max : v;
}

function smoothstep(edge0, edge1, x) {
  if (edge1 <= edge0) return x >= edge1 ? 1 : 0;
  const t = clamp((x - edge0) / (edge1 - edge0), 0, 1);
  return t * t * (3 - 2 * t);
}

export function expandRect(rect, paddingPx, maxW, maxH) {
  const x1 = clamp(rect.x - paddingPx, 0, maxW);
  const y1 = clamp(rect.y - paddingPx, 0, maxH);
  const x2 = clamp(rect.x + rect.w + paddingPx, 0, maxW);
  const y2 = clamp(rect.y + rect.h + paddingPx, 0, maxH);
  return {
    x: Math.floor(x1),
    y: Math.floor(y1),
    w: Math.max(1, Math.ceil(x2 - x1)),
    h: Math.max(1, Math.ceil(y2 - y1)),
  };
}

export function ensureCanvas(imageLike) {
  if (imageLike?.getContext?.('2d')) return imageLike;
  const copy = document.createElement('canvas');
  copy.width = imageLike.width;
  copy.height = imageLike.height;
  copy.getContext('2d').drawImage(imageLike, 0, 0);
  return copy;
}

export function cropToCanvas(image, rect) {
  const c = document.createElement('canvas');
  c.width = rect.w;
  c.height = rect.h;
  const ctx = c.getContext('2d');
  if (!ctx) return null;
  ctx.drawImage(
    image,
    rect.x, rect.y, rect.w, rect.h,
    0, 0, rect.w, rect.h,
  );
  return c;
}

export function canvasToBlobUrl(canvas) {
  return new Promise(resolve => canvas.toBlob(blob => resolve(URL.createObjectURL(blob)), 'image/png'));
}

export function imageToBlobUrl(image) {
  const c = document.createElement('canvas');
  c.width = image.width;
  c.height = image.height;
  c.getContext('2d').drawImage(image, 0, 0);
  return canvasToBlobUrl(c);
}

// True if any pixel of `image` is less than fully opaque. Used to decide
// whether the alpha-restore pass is worth running -- opaque inputs (JPEGs,
// flattened PNGs) skip it entirely. A single native-size readback; cheap
// next to inference.
export function hasTransparency(image) {
  const w = image?.naturalWidth ?? image?.videoWidth ?? image?.width ?? 0;
  const h = image?.naturalHeight ?? image?.videoHeight ?? image?.height ?? 0;
  if (w < 1 || h < 1) return false;
  const c = document.createElement('canvas');
  c.width = w;
  c.height = h;
  const ctx = c.getContext('2d', { willReadFrequently: true });
  if (!ctx) return false;
  ctx.drawImage(image, 0, 0, w, h);
  const data = ctx.getImageData(0, 0, w, h).data;
  let transparent = false;
  for (let i = 3; i < data.length; i += 4) {
    if (data[i] < 255) { transparent = true; break; }
  }
  c.width = c.height = 0;
  return transparent;
}

// Re-apply `referenceImage`'s transparency to an upscaled (opaque) result.
// SR models are RGB-only, so the engine emits fully-opaque canvases; this
// keeps the result's recovered RGB but masks it by the reference's alpha,
// upscaled to the result size. Where the reference was transparent the
// result becomes transparent again ("not represented in the final output");
// soft edges fade via smooth alpha scaling. Returns a fresh canvas.
export function applyAlphaMask(resultImage, referenceImage) {
  const w = resultImage?.width ?? 0;
  const h = resultImage?.height ?? 0;
  if (w < 1 || h < 1) return resultImage;
  const out = document.createElement('canvas');
  out.width = w;
  out.height = h;
  const ctx = out.getContext('2d');
  if (!ctx) return resultImage;
  ctx.drawImage(resultImage, 0, 0);
  // destination-in keeps the result RGB only where the (upscaled) reference
  // is opaque, multiplying the result's alpha by the reference's alpha.
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.globalCompositeOperation = 'destination-in';
  ctx.drawImage(referenceImage, 0, 0, w, h);
  return out;
}

export function blendCanvas(destCanvas, srcCanvas, opacity) {
  const alpha = clamp(opacity, 0, 1);
  if (alpha <= 0) return destCanvas;
  const ctx = destCanvas.getContext('2d');
  if (!ctx) return destCanvas;
  ctx.save();
  ctx.globalAlpha = alpha;
  ctx.drawImage(srcCanvas, 0, 0, destCanvas.width, destCanvas.height);
  ctx.restore();
  return destCanvas;
}

// Composite `patch` onto `dest` at (x, y) with a feathered alpha mask.
// `innerRect` (in patch-local coords) marks the unfeathered region; pixels
// outside it fade out over `featherPx`. Without it, feathering runs from
// the patch edges inward.
export function compositeFeathered(destCanvas, patchCanvas, x, y, {
  featherPx = 8,
  innerRect = null,
  blendOpacity = 1,
} = {}) {
  const w = patchCanvas.width;
  const h = patchCanvas.height;
  if (w < 1 || h < 1) return false;
  const opacity = clamp(blendOpacity, 0, 1);
  if (opacity <= 0) return true;
  const minDim = Math.min(w, h);
  const t = Math.max(1, Math.min(Math.floor(featherPx), Math.floor(minDim / 2)));

  const maskCanvas = document.createElement('canvas');
  maskCanvas.width = w;
  maskCanvas.height = h;
  const mctx = maskCanvas.getContext('2d');
  if (!mctx) return false;
  const mask = mctx.createImageData(w, h);
  const mpx = mask.data;
  for (let py = 0; py < h; py++) {
    for (let px = 0; px < w; px++) {
      const idx = (py * w + px) * 4;
      let a = 0;
      if (innerRect) {
        const ix1 = innerRect.x;
        const iy1 = innerRect.y;
        const ix2 = innerRect.x + innerRect.w;
        const iy2 = innerRect.y + innerRect.h;
        const ox = px < ix1 ? (ix1 - px) : px >= ix2 ? (px - ix2 + 1) : 0;
        const oy = py < iy1 ? (iy1 - py) : py >= iy2 ? (py - iy2 + 1) : 0;
        const d = Math.hypot(ox, oy);
        a = d <= 0 ? 1 : (1 - smoothstep(0, t, d));
      } else {
        const dx = Math.min(px, w - 1 - px);
        const dy = Math.min(py, h - 1 - py);
        const d = Math.min(dx, dy);
        a = smoothstep(0, t, d);
      }
      const alpha = Math.max(0, Math.min(255, Math.round(a * opacity * 255)));
      mpx[idx] = 255;
      mpx[idx + 1] = 255;
      mpx[idx + 2] = 255;
      mpx[idx + 3] = alpha;
    }
  }
  mctx.putImageData(mask, 0, 0);

  const patchMasked = document.createElement('canvas');
  patchMasked.width = w;
  patchMasked.height = h;
  const pctx = patchMasked.getContext('2d');
  if (!pctx) return false;
  pctx.drawImage(patchCanvas, 0, 0);
  pctx.globalCompositeOperation = 'destination-in';
  pctx.drawImage(maskCanvas, 0, 0);

  const dctx = destCanvas.getContext('2d');
  if (!dctx) return false;
  dctx.drawImage(patchMasked, x, y);

  maskCanvas.width = 0;
  maskCanvas.height = 0;
  patchMasked.width = 0;
  patchMasked.height = 0;
  return true;
}

function blurCanvas(src, radiusPx) {
  const w = src.width;
  const h = src.height;
  const out = document.createElement('canvas');
  out.width = w;
  out.height = h;
  const ctx = out.getContext('2d');
  ctx.filter = `blur(${radiusPx}px)`;
  ctx.drawImage(src, 0, 0);
  return out;
}

// Re-tint an SR result to the LR input's color/illumination without touching
// its detail ("wavelet" color fix): take the SR's high-frequency content and
// the reference's low-frequency content, i.e. out = sr + (blur(ref) - blur(sr)).
// SR models often drift in hue/brightness; this pins the low frequencies back
// to the source while leaving the recovered structure intact. `referenceImage`
// is the LR input (any drawable); it's upscaled to the SR size first.
//
// With `matchContrast`, the recovered detail is additionally scaled by the
// per-channel gain (refStd / srStd) measured on the low-frequency (blurred)
// layers. That nudges the result's contrast toward the input's: a model that
// upscales punchier than the source gets a gain < 1, a flatter one gets > 1.
// The mean match above is offset-only; this adds the multiplicative term.
export function matchColorToReference(
  srCanvas,
  referenceImage,
  { blurFraction = 0.02, matchContrast = false, maxGain = 2 } = {},
) {
  const w = srCanvas.width;
  const h = srCanvas.height;
  if (w < 1 || h < 1) return srCanvas;

  const ref = document.createElement('canvas');
  ref.width = w;
  ref.height = h;
  const rctx = ref.getContext('2d');
  rctx.imageSmoothingEnabled = true;
  rctx.imageSmoothingQuality = 'high';
  rctx.drawImage(referenceImage, 0, 0, w, h);

  // Blur radius scales with image size so it captures only low-frequency
  // (color/illumination) content and not edges/texture.
  const blurPx = Math.max(2, Math.round(Math.min(w, h) * blurFraction));
  const srBlur = blurCanvas(srCanvas, blurPx);
  const refBlur = blurCanvas(ref, blurPx);

  const sp = srCanvas.getContext('2d').getImageData(0, 0, w, h);
  const sbp = srBlur.getContext('2d').getImageData(0, 0, w, h).data;
  const rbp = refBlur.getContext('2d').getImageData(0, 0, w, h).data;
  const px = sp.data;

  // Per-channel detail gain from the spread of each blurred layer. Clamped so
  // a near-flat SR channel (tiny denominator) can't blow detail up to noise.
  const gain = [1, 1, 1];
  if (matchContrast) {
    const minGain = 1 / maxGain;
    for (let c = 0; c < 3; c++) {
      gain[c] = clamp(channelStd(rbp, c) / (channelStd(sbp, c) || 1e-6), minGain, maxGain);
    }
  }

  for (let i = 0; i < px.length; i += 4) {
    for (let c = 0; c < 3; c++) {
      // refBlur (matched low freq) + detail*gain (contrast-scaled high freq).
      const v = rbp[i + c] + (px[i + c] - sbp[i + c]) * gain[c];
      px[i + c] = v < 0 ? 0 : v > 255 ? 255 : v;
    }
  }

  const out = document.createElement('canvas');
  out.width = w;
  out.height = h;
  out.getContext('2d').putImageData(sp, 0, 0);

  ref.width = ref.height = 0;
  srBlur.width = srBlur.height = 0;
  refBlur.width = refBlur.height = 0;
  return out;
}

// Standard deviation of one RGBA channel (`c` in 0..2) over an Uint8 buffer.
function channelStd(data, c) {
  const n = data.length / 4;
  if (n < 1) return 0;
  let sum = 0;
  for (let i = c; i < data.length; i += 4) sum += data[i];
  const mean = sum / n;
  let varSum = 0;
  for (let i = c; i < data.length; i += 4) {
    const d = data[i] - mean;
    varSum += d * d;
  }
  return Math.sqrt(varSum / n);
}

// Feather width (output px) for compositing a detection patch back onto an
// upscaled canvas. Scales with the detected region's min dimension, clamped
// by the padding ring and patch size so the transition stays inside the pad.
export function computeFeatherPx({
  configuredFeatherPx,
  regionW,
  regionH,
  patchW,
  patchH,
  paddingPx,
  scale,
}) {
  const minOut = 4;
  const maxOut = 12;
  const configuredOut = Math.max(0, configuredFeatherPx * scale);
  const regionMinOut = Math.max(1, Math.min(regionW, regionH) * scale);
  const regionDriven = Math.round(regionMinOut * 0.015);
  let feather = clamp(
    Math.max(configuredOut, regionDriven),
    minOut,
    maxOut,
  );

  const paddingOut = Math.max(0, paddingPx * scale);
  if (paddingOut > 0) {
    feather = Math.min(feather, Math.max(4, Math.round(paddingOut * 0.9)));
  } else {
    feather = Math.min(feather, 12);
  }

  const patchLimit = Math.max(2, Math.floor(Math.min(patchW, patchH) / 4));
  return Math.max(2, Math.min(Math.round(feather), patchLimit));
}
