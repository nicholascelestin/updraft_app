...
import { BACKEND_EVENT_KIND, dispatchBackendEvent, shortenReason } from 'lib/backend-events';
import { ensureOrt } from 'lib/ort-loader';

// ───── Intent vocabulary ──────────────────────────────────────────────────
// The 'gpu' | 'cpu' enum every engine and UI talks. normalizeIntent accepts
// legacy 'webgpu' | 'wasm' forms from older localStorage so a returning user
// doesn't get a broken dropdown. Fallback differs by caller: engines default
// to CPU (safer for inference), UI defaults to GPU (the typical user intent).
...
export function normalizeIntent(value, fallback = INTENT.CPU) {
  if (value === 'webgpu' || value === INTENT.GPU) return INTENT.GPU;
  if (value === 'wasm'   || value === INTENT.CPU) return INTENT.CPU;
  return fallback;
}
...
export async function loadSession(modelBytes, intent, opts = {}) {
  if (intent !== INTENT.GPU && intent !== INTENT.CPU) {
    throw new Error(`loadSession: unknown intent ${JSON.stringify(intent)} (expected 'gpu' or 'cpu')`);
  }
  // ORT-Web is loaded on demand (not via a <script> tag) to keep its ~774 KB
  // off the initial-load path. Both arms need it: the web arm runs inference
  // through it, and the native arm wraps worker outputs in ort.Tensor.
  await ensureOrt();
...
async function loadWeb(modelBytes, intent, { profile = false, preferredOutputLocation, graphOptimizationLevel } = {}) {
  ...
  if (ort.env.webgpu) {
    ort.env.webgpu.profilingMode = (profile && intent === INTENT.GPU) ? 'default' : 'off';
  }
  ...
  if (intent === INTENT.GPU) {
    ...
    dispatchBackendEvent({ kind: BACKEND_EVENT_KIND.ATTEMPT, backend: 'web-webgpu' });
    try {
      const session = await ort.InferenceSession.create(modelBytes, sessionOpts);
      dispatchBackendEvent({ kind: BACKEND_EVENT_KIND.SUCCESS, backend: 'web-webgpu' });
      return { session, realizedBackend: 'web-webgpu' };
    } catch (e) {
      console.warn(`[backend] WebGPU failed, falling back to WASM. Reason:`, e);
      dispatchBackendEvent({ kind: BACKEND_EVENT_KIND.FALLBACK, backend: 'web-webgpu', reason: shortenReason(e) });
  ...
  dispatchBackendEvent({ kind: BACKEND_EVENT_KIND.ATTEMPT, backend: 'web-wasm' });
  const session = await ort.InferenceSession.create(modelBytes, sessionOpts);
  dispatchBackendEvent({ kind: BACKEND_EVENT_KIND.SUCCESS, backend: 'web-wasm' });
